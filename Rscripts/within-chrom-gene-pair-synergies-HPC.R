...
genotype <- fread("genotype-matrix-hg19-annotated-pheno.tsv") %>%
  select(-FID,-IID,-MAT,-PAT,-SEX) %>%
  as.data.frame()

pheno <- genotype["PHENOTYPE"]
H_D <- entropy(pheno, method = "emp")
setwd("/Users/shane/School/CU-Denver/Masters-Project/HPC-res/bayesian-intrxn/gene-pairs")
path <- paste0("BayesianInt-Res/gene_pairs/intrxn-pip_",chr,".tsv")
intrxns <- fread(path) %>%
  arrange(desc(PIP))

path2 <- paste0("BayesianInt-Res/gene_pairs/main-pip_",chr,".tsv")
main <- fread(path2) %>%
  arrange(desc(pip))
...
for(k in 1:nrow(intrxns)){
  ...
  gene1_L <- fread(paste0("within-gene-syn-res/Laplacians/", gene1, "_L_matrix.csv")) %>%
    as.data.frame() %>%
    column_to_rownames("V1") %>%
    mutate(across(everything(), as.numeric)) %>%
    as.matrix()
  
  gene2_L <- fread(paste0("within-gene-syn-res/Laplacians/", gene2, "_L_matrix.csv")) %>%
    as.data.frame() %>%
    column_to_rownames("V1") %>%
    mutate(across(everything(), as.numeric)) %>%
    as.matrix()
  ...
  write.table(L,
              file = paste0("within-gene-syn-res/Laplacians/",
  ...
  output_file <- file.path("within-gene-syn-res/", paste0("Networks/chr",chr,"_",gene1,"_",gene2, "_network.png"))
  ggsave(output_file, plot = g, width = 8, height = 6, dpi = 300, bg = "white")
  write.table(name_dict, file = paste0("within-gene-syn-res/Networks/chr",chr,"_",gene1, "_", gene2, "_SNP_dictionary.tsv"),
              sep = "\t",
              quote = FALSE,
              col.names = TRUE,
              row.names = FALSE)
...
for(k in 1:nrow(main)){
  ...
  gene_L <- fread(paste0("within-gene-syn-res/Laplacians/", gene, "_L_matrix.csv")) %>%
    as.data.frame() %>%
    column_to_rownames("V1") %>%
    mutate(across(everything(), as.numeric)) %>%
    as.matrix()
  ...
  output_file <- file.path("within-gene-syn-res/", paste0("Networks/chr",chr,"_",gene, "_network.png"))
  ggsave(output_file, plot = g, width = 8, height = 6, dpi = 300, bg = "white")
  write.table(
    name_dict,
    file = paste0("within-gene-syn-res/Networks/chr", chr, "_", gene, "_SNP_dictionary.tsv"),
...
textme(api = api$V1, project = "masters", channel = "laplacian",
       event = "Laplacian Construction", description = paste0("CHR",chr, " is done!"))

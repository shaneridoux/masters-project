...
path <- paste0("BayesianInt-Res/gene_pairs/intrxn-pip_chr",chr,".tsv")
intrxns <- fread(path) %>%
  arrange(desc(PIP))

path2 <- paste0("BayesianInt-Res/gene_pairs/main-pip_chr",chr,".tsv")
main <- fread(path2) %>%
  arrange(desc(pip))
...
for(k in 1:nrow(main)){
  ...
  ggsave(output_file, plot = g, width = 8, height = 6, dpi = 300, bg = "white")
  write.table(name_dict, file = paste0("within-gene-syn-res/Networks/chr",chr,"_",gene, "_SNP_dictionary.tsv"),
              sep = "\t",
              quote = FALSE,
              col.names = TRUE,
              row.names = FALSE)
